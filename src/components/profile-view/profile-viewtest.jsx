...
//bring user data over as a prop?
export class ProfileViewtest extends React.Component {
  constructor() {
    super();
    this.state = {
      Username: null,
  ...
  getUser = (token) => {
    const Username = localStorage.getItem("user");
    axios
      .get(`https://myflix17507.herokuapp.com/users/${Username}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        this.setState({
  ...
  onDeleteUser() {
    ...
    axios
      .delete(`https://myflix17507.herokuapp.com/users/${Username}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        console.log(response);
